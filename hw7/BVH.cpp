...
BVHBuildNode* BVHAccel::recursiveBuild(std::vector<Object*> objects)
{
    ...
    if (objects.size() == 1) {
        ...
        if (splitMethod == SplitMethod::NAIVE) {
            ...
            auto compute_bucket = [](double left, double right, double position, int n_buckets) {
            ...
            for (int dim = 0; dim < 3; ++dim) {
                ...
                for (int plane = 1; plane < BVHAccel::num_of_buckets; ++plane) {
                    ...
                    double cost = left_box.SurfaceArea() * left_num + right_box.SurfaceArea() * right_num /* * bounds.SurfaceArea()*/; 
...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
    ...
    std::array<int, 3> is_positive = {int(ray.direction.x > 0),
...
void BVHAccel::Sample(Intersection &pos, double &pdf){
    double p = std::sqrt(get_random_double()) * root->area;
    getSample(root, p, pos, pdf);
    // This is the area of the whole MeshTriangle (AreaLight)
    pdf /= root->area;
}
